...
export const createTask = ({name, desc, cycle, reminder, deadline, weight}) => {
    ...
    firebase.database().ref(`/users/${currentUser.uid}/house_id`)
        ...
        .then(function(snapshot) {
            // Get the joinCode from the user making this task
...
export const assignTask = (taskId) => {
    const { currentUser } = firebase.auth();
    getHouseId().once('value').then( function(snapshot) {
        var house_id = snapshot.val();
        console.log("ok1");

        // Get current user index
        firebase.database().ref(`/houses/${house_id}/cur_user`).once('value')
            .then((snapshot) => {
                // Assign the task to correct user
                getHouseUsers().then( function(userList) {
                    console.log("ok3");
                    ...
                    // Set current user
                    setUserTask( userList[cur_user_indx].user_id, taskId);

                    // Update current user index
                    firebase.database().ref(`/houses/${house_id}/cur_user`).set((cur_user_indx + 1) % userList.length);
                });
            });
    });
};